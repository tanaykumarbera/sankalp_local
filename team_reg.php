...
if(!(isset($_SESSION['op_id']))||(($_SESSION['event_id']!=-1)&&($_SESSION['event_id']!=0)))
    ...
    function iselected($arr,$key)
                ...
                if(iset($_REQUEST['memid_1'])) { $mem[1]=$_REQUEST['memid_1']; $boool=TRUE; }
                if(iset($_REQUEST['memid_2'])) { $mem[2]=$_REQUEST['memid_2']; $boool=TRUE; }
                if(iset($_REQUEST['memid_3'])) { $mem[3]=$_REQUEST['memid_3']; $boool=TRUE; }
                if(iset($_REQUEST['memid_4'])) { $mem[4]=$_REQUEST['memid_4']; $boool=TRUE; }
                if(iset($_REQUEST['memid_5'])) { $mem[5]=$_REQUEST['memid_5']; $boool=TRUE; }
...
<html>
    ...
    <body style="background: url('_img/team_back.jpg') fixed no-repeat;">
      <a href="logout.php"><div class="logout" title="Click to logout"></div></a> <a href="search.php"><div class="serch" title="Search Student or Team Details"></div></a> <div style="position: fixed; background: url('_img/black.png') repeat; bottom:5px; left: 69px;height: 20px; padding-top: 2px; color: white; font-family: Monospace; border-radius: 5px;">&nbsp;&nbsp;Logged in as <?php echo $_SESSION['op_name'];?>&nbsp;-© Tanay Kumar Bera, Tech-niché, Sankalp-2k13&nbsp;&nbsp;</div>
        <div style="<?php if(isset($_REQUEST['mem_sbtn'])&& $boool) echo 'margin: 2% auto;'; else echo 'margin: 20% auto;';?> width: 100%; height: 200px; background: url('_img/black.png') repeat; padding-top: 20px; color: white;">
            <font face="Calibri">
            ...
            <tr>
                ...
                <center> <input type="submit" name="mem_sbtn" value="TEAM UP" class="button blue"/></center>     
        ...
        <div style="margin-top: 20px; width: 100%;">
            ...
            <div style="position:relative; padding-left: 40%; padding-top: 20px; padding-bottom:20px; height: auto; background: url('_img/black.png') repeat; color: white;" >
                <form name="team_details" action="post_team.php" method="POST">
                <font face="Calibri"><b>
                    <?php
                        ...
                        {
                            $newbool=TRUE;
                            echo '<input type="checkbox" name="events[]" value="'.$a['event_id'].'" checked="checked" /> &nbsp;&nbsp;'.$a['event_name'].'<br/>';
                        }
                        
                      if($newbool)
                      {
                    ?>
                    <br/> TEAM NAME: &nbsp;&nbsp;<input type="text" name="team_name" class="box"/><br/>
                    <input type="submit" name="team_sbtn" value="FINALIZE TEAM" class="button blue"/>
